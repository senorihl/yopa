...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		# only run migrations on a not empty migrations directory

		if [ -d /docker-entrypoint-migrations.d/ ] && [ "$(ls -A /docker-entrypoint-migrations.d/)" ]; then
			docker_verify_minimum_env
			pg_setup_hba_conf "$@"
			export PGPASSWORD="${PGPASSWORD:-$POSTGRES_PASSWORD}"
      docker_temp_server_start "$@"
      docker_process_init_files $(find /docker-entrypoint-migrations.d/ -type f -name "*.sh" | sort -n)
			docker_temp_server_stop
      unset PGPASSWORD
			cat <<-'EOM'

				PostgreSQL migrations process complete; ready for start up.

			EOM
		else
			cat <<-'EOM'

				PostgreSQL migrations directory appears to be empty; Skipping migrations
